...
document.addEventListener('DOMContentLoaded', function() {
    ...
    new Sortable(previewContainer, {
        ...
        onEnd: function() {
            // Get the new order of files
            const files = Array.from(previewContainer.querySelectorAll('.preview-item'))
                .map(item => item.dataset.filename);
            
            // Send the new order to the server
            const formData = new FormData();
            formData.append('action', 'reorder_files');
            formData.append('table', '<?php echo htmlspecialchars($table); ?>');
            formData.append('idpk', '<?php echo htmlspecialchars($idpk); ?>');
            ...
            .then(data => {
                if (data.success) {
                    ...
                    alert('Error reordering files: ' + data.error);
                }
            })
            .catch(error => {
                console.error('Error:', error);
                alert('Error reordering files');
            });
        }
    });
...
document.addEventListener("DOMContentLoaded", () => {
    document.querySelectorAll('input[type="number"]:not([data-linked-table])').forEach(numberInput => {
        // Hide original number input initially (or when editing)
        numberInput.style.position = "relative";

        // When user focuses the number input, replace it with a text input overlay
        numberInput.addEventListener("focus", () => {
            // Hide the number input (but keep size)
            numberInput.style.visibility = "hidden";

            // Create a text input to overlay
            const textInput = document.createElement("input");
            textInput.type = "text";
            textInput.className = numberInput.className;
            textInput.classList.add("form-control");
            textInput.style.position = "absolute";
            textInput.style.margin = 0;
            textInput.style.padding = window.getComputedStyle(numberInput).padding;
            textInput.style.border = window.getComputedStyle(numberInput).border;
            textInput.style.fontSize = window.getComputedStyle(numberInput).fontSize;
            textInput.style.fontFamily = window.getComputedStyle(numberInput).fontFamily;
            textInput.style.width = numberInput.offsetWidth + "px";
            textInput.style.height = numberInput.offsetHeight + "px";
            textInput.style.left = numberInput.offsetLeft + "px";
            textInput.style.top = numberInput.offsetTop + "px";
            textInput.style.zIndex = 1000;

            // Initialize with current number input value (as string)
            textInput.value = numberInput.value || "";

            // Insert the text input right after the number input in the DOM
            ...
            // On blur of text input: evaluate, remove text input, update number input and show it again
                ...
                try {
                    ...
                    if (!isNaN(result)) {
                        ...
                        if (wasRounded && wasExpression) {
                            numberInput.setAttribute("title", `rounded (${raw})`);
                        } else if (wasRounded) {
                            numberInput.setAttribute("title", `rounded ${result}`);
                        } else if (wasExpression) {
                            numberInput.setAttribute("title", raw);
                        } else {
