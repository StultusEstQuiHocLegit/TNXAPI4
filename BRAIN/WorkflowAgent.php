...
try {
    if ($pastWorkflowId) {
        ...
        $stmtChildren = $pdo->prepare("SELECT * FROM workflows WHERE IdpkUpstreamWorkflow = ? AND IdpkOfAdmin = ?");
        $stmtChildren->execute([$pastWorkflowId, $user_id]);
        $children = $stmtChildren->fetchAll(PDO::FETCH_ASSOC);
        $workflowEntries = array_merge($workflowEntries, $children);
    }
