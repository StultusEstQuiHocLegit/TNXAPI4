...
document.addEventListener('DOMContentLoaded', function () {
    ...
    messageInput.addEventListener('keydown', function (e) {
        if (e.key === 'Enter') {
            if (e.ctrlKey || e.shiftKey) {
                ...
                e.preventDefault();
                this.dispatchEvent(new Event('input')); // trigger resize
                return;
            }
    ...
    function addMessageToChat(text, type, label = null, index = messageHistory.length, save = true, attachments = [], workflowId = null, workflowEmoji = '', workflowText = '', emailBotFlag = false, emailBotInfoText = '') {
        ...
        if (emailLikeLabels.includes(baseLabel)) {
          ...
          emailDiv.className = 'chat-bubble email';
          ...
          // wire up blur/change listeners
          [...headerInputs].forEach(input => {
            input.addEventListener('blur', () => {
              // locate our message’s index
              const idx = Array.from(chatMessages.children).indexOf(emailDiv);
              if (idx === -1) return;
              // update and persist
        ...
        if (['PDF', 'PDFINVOICE', 'PDFPURCHASE', 'PDFOFFER', 'PDFDELIVERYRECEIPT', 'PDFREPORT', 'PDFCONTRACT', 'PDFLEGALDOCUMENT'].includes(baseLabel)) {
            ...
            pdfDiv.className = 'chat-bubble pdf';
            let msgId = messageHistory[index]?.id || generateUniqueId();
            ...
            function getTimestampFilename(title) {
              ...
              const datePart = `${now.getFullYear()}-${pad(now.getMonth() + 1)}-${pad(now.getDate())}_${pad(now.getHours())}_${pad(now.getMinutes())}-${pad(now.getSeconds())}`;
              const base = typeof companyNameCompressed !== 'undefined' && companyNameCompressed ? companyNameCompressed : 'TRAMANNPROJECTS';
              return title ? `${base}_${title}_${datePart}.pdf` : `${base}_${datePart}.pdf`;
            }
        ...
        // if a label was provided, show it as a badge
        if (label) {
        ...
        // load Chatdbsearch previews if the label starts with 'CHATDBSEARCH'
        if (baseLabel.startsWith('CHATDBSEARCH')) {
          const previewContainer = document.createElement('div');
          previewContainer.className = 'search-dbpreview-container';
          fetch(`ChatdbsearchPreviews.php?label=${encodeURIComponent(label)}`)
            .then(response => response.text())
            .then(html => {
              previewContainer.innerHTML = html;
            })
            .catch(err => {
              console.error('Error loading search previews:', err);
              previewContainer.textContent = 'Failed to load search previews.';
            });
          messageDiv.appendChild(previewContainer);
        }
        
        // Check if the text is a chart configuration
            ...
            try {
                ...
                if (chartConfig.type && chartConfig.data) {
                    ...
                    if (save) {
                        ...
                        // only save if it’s not a repeat of the last message
                        const last = messageHistory[messageHistory.length - 1];
                        const isDuplicate = last && last.text === text && last.type === type && last.label === label;
                        if (!isDuplicate) {
                          messageHistory.push({ id: msgId, text, type, label, attachments, workflowId, workflowEmoji, workflowText, emailBotFlag });
                          if (messageHistory.length > MAX_MESSAGES) {
                            messageHistory = messageHistory.slice(-MAX_MESSAGES);
        ...
        // Parse markdown content for non-chart messages
        const contentDiv = document.createElement('div');
        
        // Special handling for math results
        if (text.trim().startsWith('= ')) {
            const result = text.trim().substring(2);
            // decide if it's a number and negative
            const isNeg = !isNaN(result) && parseFloat(result) < 0;
            const cls   = isNeg ? ' negative' : '';
            contentDiv.innerHTML =
              `<span class="math-result">= <span class="math-value${cls}">${result}</span></span>`;
        } else {
            contentDiv.innerHTML = marked.parse(text);
            contentDiv.querySelectorAll('span.suggestions').forEach(span => {
                span.title = 'click to execute directly';
                span.addEventListener('click', () => {
                    sendSuggestion(span.textContent);
                });
            });
        }
        ...
        const historyEntry = messageHistory[index];
        const attList = (historyEntry && Array.isArray(historyEntry.attachments))
          ? historyEntry.attachments
          : attachments;

        let attachmentsWrap = null;
        const hasAttachments = attList && attList.length > 0;
        const hasEmailBot = emailBotFlag || emailBotInfoText;
        if (hasAttachments || (workflowId && workflowEmoji) || hasEmailBot) {
          attachmentsWrap = document.createElement('div');
          attachmentsWrap.className = 'chat-attachments';
        }

        if (emailBotFlag) {
          const botDiv = document.createElement('div');
          botDiv.className = 'chat-attachment workflow-attachment';
          botDiv.title = 'EMAIL BOT will handle this task';
          const botSpan = document.createElement('span');
          botSpan.textContent = '🤖';
          botSpan.style.fontSize = '1.5rem';
          botDiv.appendChild(botSpan);
          if (attachmentsWrap) {
            attachmentsWrap.appendChild(botDiv);
          } else {
            attachmentsWrap = document.createElement('div');
            attachmentsWrap.className = 'chat-attachments';
            attachmentsWrap.appendChild(botDiv);
        ...
        if (emailBotInfoText) {
          ...
          infoDiv.className = 'chat-attachment workflow-attachment';
          infoDiv.title = emailBotInfoText;
          const infoSpan = document.createElement('span');
          infoSpan.textContent = '📃';
          infoSpan.style.fontSize = '1.5rem';
          infoDiv.appendChild(infoSpan);
          if (attachmentsWrap) {
            attachmentsWrap.appendChild(infoDiv);
          } else {
            attachmentsWrap = document.createElement('div');
            attachmentsWrap.className = 'chat-attachments';
            attachmentsWrap.appendChild(infoDiv);
            ...
            if (baseLabel.startsWith('CHATDB') || baseLabel.startsWith('DATABASES') || baseLabel.startsWith('BUYING') || baseLabel.startsWith('SELLING')) {
              ...
              messageDiv.querySelectorAll('table').forEach(table => {
               ...
               table.querySelectorAll('td').forEach(td => {
                ...
                // Add CSV download if table is present
                const csvBtn = document.createElement('button');
                csvBtn.textContent = '🔽';
                csvBtn.title = 'download CSV';
                ...
                csvBtn.onclick = () => {
                  ...
                  rows.forEach(row => {
                    const cols = row.querySelectorAll('th, td');
                    const line = Array.from(cols).map(td => {
                      const cleanText = td.textContent.replace(/["⇅↑↓]/g, '').trim();
                      return `"${cleanText.replace(/"/g, '""')}"`;
                    }).join(',');
                    csv += line + '\n';
                  });
                  ...
                  const pad = n => n.toString().padStart(2, '0');
                  const base = typeof companyNameCompressed !== 'undefined' && companyNameCompressed ? companyNameCompressed : 'TRAMANNPROJECTS';
                  const filename = `${base}_${now.getFullYear()}-${pad(now.getMonth() + 1)}-${pad(now.getDate())}_${pad(now.getHours())}-${pad(now.getMinutes())}-${pad(now.getSeconds())}.csv`;
                  a.download = filename;
                  document.body.appendChild(a);
                  a.click();
                  document.body.removeChild(a);
...
const tableMenu = document.getElementById('custom-table-menu');

let currentCell = null;

document.addEventListener('contextmenu', function (e) {
    const td = e.target.closest('td, th');
    if (!td || !td.closest('table.sortable-markdown-table')) return;
    ...
    const actions = [
        { label: '➕ ADD ROW BELOW', action: () => modifyTable('add-row') },
        { label: '➕ ADD COLUMN RIGHT', action: () => modifyTable('add-col') },
    ];
    if (rowCount > 2) {
        actions.push({ label: '❌ REMOVE ROW', action: () => confirmAndModify('delete-row') });
    }
    if (colCount > 1) {
        actions.push({ label: '❌ REMOVE COLUMN', action: () => confirmAndModify('delete-col') });
    }
