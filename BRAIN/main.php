...
// # HOW PROMPT CONSTRUCTION WORKS FOR BUYING AND SELLING ACTIONS WORKS
// Whenever we got the labels: pdfinvoice, pdfpurchase, pdfdeliveryreceipt or incominggoodsinspection, we added accordingly: selling, buying, selling, buying as additonal labels before them,
// if the users enters directly into the databases, we don't use the normal databasesinsertinto, but instead buying/selling directly.
// 
// 
// # INBOUND / BUYING
// 
// ## UPLOADS INVOICE
// -> look for purchase entries, if missing, create corresponding database entries (no goods arrived yet and unpaid, unless user says otherwise)
// 
// ## UPLOADS DELIVERY RECEIPT
// -> create table for inspection of incoming goods with: article number / idpk + name + quantity in subsequent delivery + quantity should + quantity is
// -> look for purchase entries, if missing, create corresponding database entries
// (then the user can later update which goods arrived and in what quantities)
// 
// ## INSERT PURCHASE DIRECTLY INTO DATABASE
// -> create corresponding database entries (no goods arrived yet and unpaid, unless user says otherwise)
// 
// 
// # OUTBOUND / SELLING
// 
// ## CREATES PDF INVOICE
// -> look for sale entries, if missing, create corresponding database entries (no goods sent yet and unpaid, unless user says otherwise)
// 
// ## CREATES PDF DELIVERY RECEIPT
// -> look for sale entries, if missing, create corresponding database entries
// (then the user can later update which goods were sent and in what quantities)
// 
// ## INSERT SALE DIRECTLY INTO DATABASE
// -> create corresponding database entries (no goods sent yet and unpaid, unless user says otherwise)
// 
// 
// # GENERAL NOTES
// - especially in the cases, where we aren't inserting into the database directly, we have to be very, very careful do not create duplicate entries
// - the buying and selling actions are added as additional information directly before each corresponding main system prompt
// - be careful with changing and adding master data such as suppliers/customers and products/services,
// try to find these informations in the database context, but don't add them if they are missing, unless the user says otherwise
// - everything else, for example to create corresponding, additional PDFs or email can be done using workflows
...
$actionTypes = $finalTypes;
$actionInstructions = $finalInstructions;
